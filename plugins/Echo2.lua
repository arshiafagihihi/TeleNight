...
local function run(msg, matches)
  if matches[1] == 'بگو' and matches[2] == 'tag' and matches[3] then
...
if matches[1] == 'بگو>' then
  ...
  local file = io.open("./Umbrella-Cp/data/بگو/["..msg.from.id.."]|"..matches[2], "w")
  file:write(text)
  file:flush()
  file:close()
  send_document("chat#id"..msg.to.id,"./umbrella/بگو/["..msg.from.id.."]|"..matches[2], ok_cb, false)
end
if matches[1] == 'بگو' then
...
return {
  ...
  usage = "!بگو [whatever]: بگوes the msg",
  patterns = {
    "^(بگو>) +(.+) (.*)$",
    "^(بگو) +(.+)$",
    "^(بگو) (tag) +(.+)$",
